...
function updateCart() {
    ...
    if (cart.length > 0) {
        ...
        for (let i in cart) {
            ...
            let cartItem = c('.model .cart--item').cloneNode(true);
            ...
            cartItem.querySelector('.cart--tem-nome').innerHTML = pizzaName ;
        ...
        c('.subtotal span:last-child').innerHTML = `R$ ${subTotal.toFixed()}`;
        c('.desconto span:last-child').innerHTML = `R$ ${desconto.toFixed()}`;
        c('.total span:last-child').innerHTML = `R$ ${total.toFixed()}`;
